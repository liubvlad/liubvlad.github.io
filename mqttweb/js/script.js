...
function onMessageArrived(message) {
  ...
  // напрямую зависит от index.html & colorpicker.js
  if (topic == "ESP_1/HEX"){  //hexcolor
    $('#colorpickerHolder').ColorPickerSetColor(data);
    setRingColor(data);
    animateLedRing();
...
/*
function _hexClicked(data){
  // data -> цвет в формате hex
  setRingColor("#" + data);
  animateLedRing();

  publish("ESP_1/HEX", "#" + data, 0, false);
}*/

function _hexClicked(data, rgbcol){
  // data -> цвет в формате hex
  setRingColor("#" + data);
  animateLedRing();
  ...
  publish("ESP_1/R", ""+rgbcol.r, 0, false); // @ temporary
  publish("ESP_1/G", ""+rgbcol.g, 0, false); // @ temporary
  publish("ESP_1/B", ""+rgbcol.b, 0, false); // @ temporary
}
